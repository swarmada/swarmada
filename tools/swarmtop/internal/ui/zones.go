...
func (m Model) viewZones() string {
	...
	if len(m.fleet.Zones) == 0 {
		...
		b.WriteByte('\n')
		b.WriteString(m.helpLine("[r] robots  [t] tasks  [a] adapters  [?] keys  [q] quit"))
		return b.String()
	}
	...
	b.WriteByte('\n')
	b.WriteString(m.helpLine("[↑↓] move  [s] split  [enter] detail  [r] robots  [t] tasks  [a] adapters  [/] filter  [?] keys"))
	return b.String()
}
