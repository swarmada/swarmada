...
func RobotColumns() []table.Column {
	return []table.Column{
		...
		// ACTION, matching what the Robot CRD calls this field — its print column
		// is name="Action", JSONPath .status.assignedAction (api/v1/robot_types.go).
		// It was "TASK" from before the FleetAction/FleetTask split, which also
		// collided with the TASK column on the tasks screen, where TASK means the
		// OWNING composite — the opposite level of the hierarchy.
...
func RobotRow(f k8sclient.Fleet, r k8sclient.RobotView, _ time.Time) table.Row {
	...
	return table.Row{
		...
		eventBadge(f, r.Name),
		actionCell(f, r.AssignedAction),
	}
}

// actionCell renders the robot's currently assigned action, resolved against the
// same snapshot the row came from — the actions are already in hand, so there is
// no join cost. A child generated by a FleetTask is shown by its SHORT member
// name: children are named "<task>-<member>", which at this width truncates to
// the task prefix and reads identically for every member of that task, so the
// cell could not say which step the robot was on. The OWNING task is deliberately
// not here — the robot detail pane answers that, the same way the tasks screen's
// narrow pane defers to detail (spec §3a).
func actionCell(f k8sclient.Fleet, assigned string) string {
	if assigned == "" {
		return "—"
	}
	for i := range f.Actions {
		if f.Actions[i].Name == assigned {
			return format.MemberName(assigned, f.Actions[i].OwnerTask)
		}
	}
	// Assigned but absent from this snapshot (filtered out, or the informer has
	// not caught up): show the raw name. A dash would read as "unassigned".
	return assigned
}
