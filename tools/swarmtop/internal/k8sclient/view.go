...
type Fleet struct {
	// Robots, Actions, Probes, Adapters, Zones and Tasks are all sorted by Name
	...
	// Tasks are the composite FleetTasks. An Action generated by one still
	// appears in Actions — grouping is the UI's job, via OwnerTask — so nothing
	// here is a second copy of an action.
	Tasks []FleetTaskView
...
type FleetActionView struct {
	...
	// OwnerTask is the composite FleetTask that generated this action, read from
	// the swarmada.io/fleettask label; empty means standalone. It is the whole
	// distinction between a member and a standalone action — every other field
	// is produced identically either way, and the UI must not infer more.
	OwnerTask string
}

// FleetTaskView is the reduced projection of one composite FleetTask (RFC-0001
// §9.1.5). Every field comes off the task's own spec/status — members included —
// so rendering a task requires no join against the FleetAction list.
type FleetTaskView struct {
	Name string

	// Phase is status.phase (Pending, Running, Succeeded, Failed, Compensating,
	// Compensated, Cancelled).
	Phase string

	// ActionSummary is status.actionSummary, the controller's "N/M Succeeded"
	// projection. Empty until the controller has computed one.
	ActionSummary string

	// DesiredState is spec.desiredState. It is task-owned and fanned out onto
	// non-terminal members, which is why a member never holds intent of its own.
	DesiredState string

	// CompletionPolicy and FailurePolicy are spec fields shown in task detail.
	CompletionPolicy string
	FailurePolicy    string

	// Members projects status.actions[] in the order the controller records it.
	Members []FleetTaskMemberView

	// CurrentMember is the member name the task row headlines: the most advanced
	// non-terminal member. Empty when the task is terminal, or when no member is
	// in one of the ranked phases.
	CurrentMember string

	// StartedAt and CompletionTime are status timestamps; the *Unknown flags
	// distinguish "not yet" from a zero time so the UI never renders the epoch.
	StartedAt             time.Time
	StartedAtUnknown      bool
	CompletionTime        time.Time
	CompletionTimeUnknown bool

	// CreatedAt is metadata.creationTimestamp — the source of kubectl's Age print
	// column. It needs no companion Unknown flag: unlike the two status timestamps
	// above, which are optional pointers, the API server always sets this, and
	// format.Age already renders a zero time as "—" rather than dating the task to
	// the epoch.
	CreatedAt time.Time
...
// FleetTaskMemberView is one status.actions[] entry — the task's own record of a
// member, which is distinct from that member's child FleetAction row in
// Fleet.Actions. ActionRef is the link between the two.
type FleetTaskMemberView struct {
	Name              string
	ActionRef         string
	Phase             string
	AssignedRobot     string
