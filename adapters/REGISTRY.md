...
| Value | Meaning |
| :---- | :------ |
| `passing` | Passes the conformance suite for the listed contract version. |
| `partial` | Implements the required (safety-critical) subset; some optional areas unverified. |
| `wip` | Under development; not yet conformant. |
| `template` | A skeleton/example, not a usable adapter. |
...
The **Protocol** column carries the fleet-adapter **contract version** (semver, e.g.
...
> **Suite revision.** Rows are qualified against the **current** conformance suite —
> **C0–C16, 43 checks** — recorded together with the contract version the result was
> earned against. This is the version-bound discipline
> [ADR-0032](../docs/adr/0032-fleet-adapter-contract-versioning-and-conformance-gating.md)
> applies to the contract version, applied to the suite revision.

| Adapter | Interface / class | Repository | Maintained by | Protocol | Conformance |
| :------ | :---------------- | :--------- | :------------ | :------- | :---------- |
...
1. Run the [conformance suite](CONFORMANCE.md) against your adapter and record the
   ...
   Put that semver in the **Protocol** column, with the wire-package identity in
